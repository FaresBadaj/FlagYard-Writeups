...
**Challenge Name**: Mischenger
**Category**: Web
**Difficulty**: Medium
**Platform**: FlagYard

## Challenge Overview:
Mischenger is a Socket.IO messenging web app with an "Export chats" feature.
To retrieve the flag we chain three flaws: a *guessable identifier* (every
`user_id` is the MD5 of the account's registration timestamp), an *information
leak* (`/api/users/stats` exposes the seeded admin's registration time, so the
admin's `user_id` is reproducible offline), and an *IDOR on the Socket.IO
export* (`export_chats` exports the chats of whichever `user_id` the caller
asks for, without ever verifying ownership). The generated admin CSV — which
would be `403` to reach through the chat API directly — contains the flag in
the admin's private "Admin Private Notes" chat.

### Steps Involved:

1. **Enumeration**:
   - The app is a Flask + Socket.IO client; REST lives under `/api/*`:
     `POST /api/auth/register`, `POST /api/auth/login`,
     `GET /api/auth/validate`, `GET /api/chats`,
     `POST /api/chats/create`, `GET /api/chats/<id>/messages`,
     `GET /api/users/stats`, `GET /api/exports`,
     `GET /api/download/<export_id>`.
   - Socket.IO events: `export_chats`, `export_completed`, `export_error`.
   - The seeded `admin` user owns "Chat with System Administrator" (chat id 2);
     chat 1, "Admin Private Notes", answers `403 Access denied` for a normal
     user.

2. **user_id = md5(registration time)**:
   - Registering answers with a JWT (`HS256`) whose payload is
     `{"user_id": <32 hex>, "exp": ...}`. The value is not a random UUID.
   - Registering twice and looking at the rounded server `Date` header shows
     both accounts map to `md5("YYYY-MM-DD HH:MM:SS")` taken at the exact
     second the row was created (the app seeds ids with
     `md5(strftime('%Y-%m-%d %H:%M:%S'))`).
   - We verified the property against our own accounts before trusting it for
     the admin.

3. **Recovering admin_id**:
   - `GET /api/users/stats` (Bearer token required) leaks each user's
     registration time. The seeded admin row is
     `2024-01-01 08:00:00`, so:
     ```
     admin_id = md5("2024-01-01 08:00:00")
              = 34fe3e505a9472469cbcc0bfa19103bd
     ```

4. **Socket.IO export IDOR**:
   - The Export UI emits `export_chats` with `{user_id, export_type:"all"}`.
   - The Socket.IO handler authenticates the connection through the **Flask
     session cookie** (a socket without the cookie answers
     `{"error": "Invalid request"}`), but it never checks that the requested
     `user_id` belongs to that session — classic IDOR.
   - Emitting `export_chats` with the admin `user_id` and a logged-in session
     cookie triggers `export_completed` → `chat_export_admin.csv`.
   - The raw Socket.IO/Engine.IO v4 handshake is: GET open (`EIO=4&transport=
     polling`) → parse `sid` → POST `40` (socket.io CONNECT) → poll for
     `40{"sid":...}` ack, answer `ping` (`2`) with `pong` (`3`) while polling
     for the `42[...]` event frame. python-socketio automates this.

5. **Export listing + download IDOR**:
   - `GET /api/exports` lists exports for *any* authenticated user.
   - `GET /api/download/<export_id>` fetches `chat_export_admin.csv`; the
     exported "Admin Private Notes" chat contains:
     ```
     ??? FLAG: FlagY{97dfb21a87614ea4fefacd206da3e6cd}
     ```

### Key Endpoints:
- `POST /api/auth/register`: Leaks the JWT `user_id` and the server `Date`
  header proving `user_id = md5(reg second)`.
- `GET /api/users/stats`: Leaks the admin's registration time
  (`2024-01-01 08:00:00`).
- `/socket.io/?EIO=4&transport=polling` + `export_chats`: Socket.IO IDOR that
  renames the victim (admin) arbitrarily.
- `GET /api/exports` / `GET /api/download/<id>`: IDOR fetch of the generated
  admin CSV.

### Techniques Used:
- **Broken ID generation**: `user_id = md5(registration second)` is fully
  guessable once the timestamp is known.
- **Info disclosure**: `/api/users/stats` discloses seeded admin registration
  time.
- **IDOR on Socket.IO RPC**: the `export_chats` event trusts a client-supplied
  `user_id` and never authorizes it against the socket owner.
- **IDOR on resources**: exports list/download are usable with any valid
  token, exposing the admin chat CSV that the chat API itself forbids.

### Tools & Libraries:
- **Python 3**: `http.client` for REST, `python-socketio` (polling transport)
  for the Socket.IO export event.
- **JWT decode**: base64url decode of the payload (no signature needed).
...
### Conclusion:
Mischenger's flag falls out of three small mistakes working together: ids built
from `md5(timestamp)` are guessable the moment you know (or can leak) a user's
registration second, `/api/users/stats` hands out exactly that seed for the
admin, and the Socket.IO `export_chats` handler never checks who is asking for
whose chats — it only demands any logged-in session. Once the CSV export is
generated for the admin, the exports list and download endpoints are equally
permissive, so the chat that a normal user could never read (403) is delivered
as a downloadable file. Defense takeaways: never derive object ids from
predictable data (use proper random UUIDs), keep per-user stats private, and
authorize every resource/RPC against the authenticated principal — including
"client-supplied" target ids on Socket.IO events.

## Credit

- **Author:** [Fares Badaj](https://www.linkedin.com/in/FaresBadaj) — [@ptok3](https://t.me/ptok3)
- **Writeup technique reference:** [Dr.Kasbr — FlagYard web/Mischenger](https://drkasbr.medium.com/flagyard-web-mischenger-d0a587973fa9)